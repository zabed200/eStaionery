...
$currency = "USD";
// $transaction = "ACRAF23DB3C4";
$transaction = strtoupper(bin2hex(random_bytes(8)));
$status = "completed";
...
// echo '<pre>'; print($_SESSION['item_total']); echo '</pre>';

foreach ($_SESSION as $name => $value) {

    // echo '<pre>'; print($name); echo '</pre>';
...
$sub = $row['product_price']*$value;
$item_quantity +=$value;
$product_remaining = $row['product_quantity'] - $_SESSION[$name];
// echo $product_remaining;

$edit_quantity = query("UPDATE products SET product_quantity=$product_remaining WHERE product_id = " . escape_string($id). " ");
confirm($edit_quantity);
